...
function makeDeps(overrides?: {
  ...
  return { authStorePath: STORE, safeStorage, fs };
}
...
describe('handleAuthSetToken', () => {
  ...
  it('encrypts and writes when safeStorage is available', async () => {
    const res = await handleAuthSetToken(deps, { token: 'ghp_secrettoken' });
  ...
  it('never echoes the token in the response', async () => {
    const res = await handleAuthSetToken(deps, { token: 'ghp_supersecret' });
  ...
  it('fails closed when safeStorage is unavailable (no plaintext fallback)', async () => {
    ...
    const res = await handleAuthSetToken(deps, { token: 'whatever' });
  ...
  it('rejects empty / oversized tokens at the schema layer', async () => {
    expect((await handleAuthSetToken(deps, { token: '' })).ok).toBe(false);
    expect((await handleAuthSetToken(deps, { token: 'x'.repeat(257) })).ok).toBe(false);
    expect((await handleAuthSetToken(deps, {})).ok).toBe(false);
  });
  ...
  it('surfaces fs write errors', async () => {
    ...
    const res = await handleAuthSetToken(deps, { token: 'tok' });
    expect(res.ok).toBe(false);
...
describe('handleAuthClearToken', () => {
  it('removes the file when present', async () => {
    ...
    const res = await handleAuthClearToken(deps, {});
    expect(res.ok).toBe(true);
  ...
  it('accepts undefined raw payload', async () => {
    const deps = makeDeps();
    const res = await handleAuthClearToken(deps, undefined);
    expect(res.ok).toBe(true);
  });
});
